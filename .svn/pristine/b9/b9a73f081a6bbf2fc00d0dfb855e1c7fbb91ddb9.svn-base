...
namespace AMC_Test
{
    public partial class HMI_Main : Form
    {
        ...
        {
            Send_string(LD[0].LD_Client, "doTask deltaHeading " + textBox5.Text);
        }
        ...
        private void Zigbee_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            try
            {
                //byte[] arr_byte = new byte[Read_cnt];
                //Zigbee.Read(arr_byte, 0, Read_cnt);

                for (int i = 0; i < Read_cnt - 1; i++)
                {
                    if (Recive_Byte[0] == 0x02)
                    {
                        Recive_Byte[recive_cnt++] = (byte)Zigbee.ReadByte();
                    }
                    else
                    {
                        Recive_Byte[0] = (byte)Zigbee.ReadByte();
                    }

                    if (recive_cnt == 18 && Recive_Byte[0] == 0x02)
                    {
                        recive_cnt = 0;
                        if (Data_Paser()== false)
                        {
                            //Insert_Err_Q("DataRecive Error");
                        }
                        else
                        {
            ...
            {
                MessageBox.Show(ex.Message, "Zigbee_DataReceived");
            }
        }
        ...
        private bool Data_Paser()
        {
            ...
            try
            {
                ...
                if (Recive_Byte[15] == (byte)arr_str[arr_str.Length - 2] && Recive_Byte[16] == (byte)arr_str[arr_str.Length - 1])
                {
                    if(arr_byte[8] == Recive_Byte[1] && arr_byte[9] == Recive_Byte[2] && arr_byte[10] == Recive_Byte[3] && arr_byte[11] == Recive_Byte[4])
                    {
                        if(Recive_Byte[5] == 0x41 && Recive_Byte[6] == 0x43 && Recive_Byte[7] == 0x4b)
                        {
        ...
        {
            if(is_serial_send == true)
            {
                if(Serial_Timer_Cnt++ >= 10)
                {
                    Serial_Timer_Cnt = 0;
                    if (Serial_Retry_Cnt++ < 5)
                    {
                        btn_open_Click("Serial_Timer_Tick", e);
                    }
                    else
                    {
                        // serial send time out error;;
                        Serial_Retry_Cnt = 0;
                        Insert_Err_Q("Send Time Out Error");
                    }
                }
            }            
        }
        ...
        private bool Chk_DO()
        {
            ...
            if (Gate1_st[0] == false && Gate1_st[1] == true)
            {
                Send_Close(AMC_Test.Properties.Settings.Default.Gate1_Add);
            }    
            else if(Gate1_st[0] == true && Gate1_st[1] == false )
            {
                Send_Open(AMC_Test.Properties.Settings.Default.Gate1_Add);                
            }
           
            if (Gate2_st[0] == false && Gate2_st[1] == true)
            {
                Send_Open(AMC_Test.Properties.Settings.Default.Gate2_Add);
            }                
            
            if (Gate3_st[0] == false && Gate3_st[1] == true)
            {
                Send_Open(AMC_Test.Properties.Settings.Default.Gate3_Add);
            }                
           
            if (Gate4_st[0] == false && Gate4_st[1] == true)
            {
                Send_Open(AMC_Test.Properties.Settings.Default.Gate4_Add);
            }                         
        ...
        private void Send_Open(string Addr)
        {
            byte[] arr_byte = new byte[19];
            byte res = 0;
            char[] arr_addr = new char[4];

            arr_addr = Addr.ToCharArray();

            char[] arr_AVG = new char[4];            
            string str1 = "";

            for(int i=0; i<4- AMC_Test.Properties.Settings.Default.LD_Number.Length; i++)
            {
                str1 += '0';
            }

            str1 += AMC_Test.Properties.Settings.Default.LD_Number;

            arr_AVG = str1.ToCharArray();

            arr_byte[0] = 0x02;
            arr_byte[1] = (byte)arr_addr[0]; //0x30; // addr >>
            arr_byte[2] = (byte)arr_addr[1]; //0x34;
            arr_byte[3] = (byte)arr_addr[2]; //0x30;
            arr_byte[4] = (byte)arr_addr[3]; //0x31; // << addr
            arr_byte[5] = 0x41;
            arr_byte[6] = 0x42;
            arr_byte[7] = 0x5A;
            arr_byte[8] = (byte)arr_AVG[0];//0x30;
            arr_byte[9] = (byte)arr_AVG[1];//0x30;
            arr_byte[10] = (byte)arr_AVG[2];//0x30;
            arr_byte[11] = (byte)arr_AVG[3];//0x38;
            arr_byte[12] = 0x30;
            arr_byte[13] = 0x49;
            arr_byte[14] = 0x30;
            arr_byte[15] = 0x30;
        ...
        private void Send_Close(string Addr)
        {
            //byte[] arr_byte = new byte[19];
            byte res = 0;
            char[] arr_addr = new char[4];
            ...
            arr_byte[6] = 0x42;
            arr_byte[7] = 0x5A;
            arr_byte[8] = (byte)arr_AVG[0];//0x30;0x30;
            arr_byte[9] = (byte)arr_AVG[1];//0x30;0x30;
            arr_byte[10] = (byte)arr_AVG[2];//0x30; 0x30;
            arr_byte[11] = (byte)arr_AVG[3];//0x38; 0x38;
            arr_byte[12] = 0x30;
            arr_byte[13] = 0x4F;
            arr_byte[14] = 0x30;
            arr_byte[15] = 0x30;
        ...
        public void Set_io_lamp(string IO, string num, string st)
        {
            ...
            if (IO == "Input:")
            {
                ...
                {
                    pb_i1.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "2")
                {
                    pb_i2.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "3")
                {
                    pb_i3.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "4")
                {
                    pb_i4.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "5")
                {
                    pb_i5.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "6")
                {
                    pb_i6.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "7")
                {
                    pb_i7.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "8")
                {
                    pb_i8.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "9")
                {
                    pb_i9.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "10")
                {
                    pb_i10.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "11")
                {
                    pb_i11.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "12")
                {
                    pb_i12.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "13")
                {
                    pb_i13.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "14")
                {
                    pb_i14.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "15")
                {
                    pb_i15.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
                else if (num.Substring(1, num.Length - 1) == "16")
                {
                    pb_i16.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_green);
                }
            }
            else
            {
                if (num.Substring(1, num.Length - 1) == "1")
                {
                    pb_o1.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o1.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "2")
                {
                    pb_o2.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o2.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "3")
                {
                    pb_o3.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o3.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "4")
                {
                    pb_o4.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o4.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "5")
                {
                    pb_o5.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o5.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "6")
                {
                    pb_o6.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o6.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "7")
                {
                    pb_o7.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o7.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "8")
                {
                    pb_o8.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o8.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "9")
                {
                    pb_o9.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o9.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "10")
                {
                    pb_o10.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o10.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "11")
                {
                    pb_o11.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o11.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "12")
                {
                    pb_o12.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o12.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "13")
                {
                    pb_o13.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o13.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "14")
                {
                    pb_o14.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o14.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "15")
                {
                    pb_o15.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
                    pb_o15.Tag = (st == "off" ? "off" : "on");
                }
                else if (num.Substring(1, num.Length - 1) == "16")
                {
                    pb_o16.Image = (st == "off" ? AMC_Test.Properties.Resources.circle_grey : AMC_Test.Properties.Resources.circle_red);
            ...
            while(b_bg_deltaHeading)
            {
                switch (ndeltaHeading_Task_cnt)
                {
                    case 1:
                        ...
                        if(double.Parse((LD[0].LD_ST.LD_LDS1_DISTANCE == null ? "10000" : LD[0].LD_ST.LD_LDS1_DISTANCE)) < 350 && double.Parse((LD[0].LD_ST.LD_LDS1_DISTANCE == null ? "10000" : LD[0].LD_ST.LD_LDS1_DISTANCE)) < 350 && n_move_1000_comp == 3)
                        {
                            Sleep_val = 3000;
                            n_move_1000_comp = 0;
                            ndeltaHeading_Task_cnt++;
